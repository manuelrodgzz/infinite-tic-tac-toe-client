<script setup>
  ...
  const showOponentAbandoned = computed(() => {
    return Boolean(
      router.currentRoute.value.query.oponentAbandoned
    )
  })
...
<template>
  <Layout>
    ...
    <OponentAbandoned
      :visible="showOponentAbandoned"
      @close="handleOponentAbandonedClose"
    />
  </Layout>
</template>
...
<style>
  ...
  .p-tabview-panel .p-float-label, .p-tabview-panel button {
    margin-top: 2rem;
  }
</style>