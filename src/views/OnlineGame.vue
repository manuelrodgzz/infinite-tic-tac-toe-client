<script setup>
  ...
  const playersArr = computed(() => {
    return Object.entries(players.value).map(
      ([id, data]) => ({id, ...data})
    )
  })

  const showTicTacToe = computed(() => {
    const arePlayersReady = playersArr.value.length > 1
    && playersArr.value.every(player => player.isReady)
    ...
    return (
      ...
      || (
        isNotTheFirstMatch
        && !players.value[playerId].isReady
      )
    )
  })
